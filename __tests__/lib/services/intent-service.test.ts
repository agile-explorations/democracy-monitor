...
function makeStatement(
  overrides: Partial<IntentStatement> & { text: string; type: 'rhetoric' | 'action'; policyArea: IntentStatement['policyArea'] }
): IntentStatement {
  return {
    source: 'test',
    sourceTier: 1,
...
describe('scoreStatements', () => {
  it('returns democratic-leaning overall for purely democratic rhetoric', () => {
    const statements: IntentStatement[] = [
      makeStatement({ text: 'We believe in the rule of law and accountability and no one is above the law and checks and balances and constitutional limits and separation of powers and transparency', type: 'rhetoric', policyArea: 'rule_of_law' }),
      makeStatement({ text: 'We comply with the courts and support accountability and transparency', type: 'rhetoric', policyArea: 'rule_of_law' }),
      makeStatement({ text: 'We protect freedom of speech and first amendment and due process and constitutional rights and bill of rights and right to protest and equal protection', type: 'rhetoric', policyArea: 'civil_liberties' }),
      makeStatement({ text: 'We support due process and equal protection and constitutional rights and bill of rights', type: 'action', policyArea: 'civil_liberties' }),
    ];
  ...
  it('returns authoritarian-leaning for authoritarian rhetoric', () => {
    const statements: IntentStatement[] = [
      makeStatement({ text: 'I have total immunity and absolute authority under article ii', type: 'rhetoric', policyArea: 'rule_of_law' }),
      makeStatement({ text: 'Lock them up, they are the enemy of the people', type: 'rhetoric', policyArea: 'civil_liberties' }),
      makeStatement({ text: 'We will terminate the constitution if needed', type: 'action', policyArea: 'civil_liberties' }),
    ];
    const result = scoreStatements(statements);
  ...
  it('produces a gap when rhetoric and actions diverge', () => {
    const statements: IntentStatement[] = [
      // Democratic rhetoric
      makeStatement({ text: 'We believe in the rule of law and checks and balances', type: 'rhetoric', policyArea: 'rule_of_law' }),
      // Authoritarian action
      makeStatement({ text: 'Fired inspector general and bypassed oversight', type: 'action', policyArea: 'rule_of_law' }),
    ];
    const result = scoreStatements(statements);
  ...
  it('returns all 5 policy areas', () => {
    const statements: IntentStatement[] = [
      ...
      makeStatement({ text: 'civil liberties protected', type: 'rhetoric', policyArea: 'civil_liberties' }),
      ...
      makeStatement({ text: 'independent agencies', type: 'rhetoric', policyArea: 'institutional_independence' }),
    ];
    const result = scoreStatements(statements);
  ...
  it('confidence scales with statement count and area diversity', () => {
    ...
    const manyStatements: IntentStatement[] = Array.from({ length: 20 }, (_, i) =>
      makeStatement({
        ...
        policyArea: ['rule_of_law', 'civil_liberties', 'elections', 'media_freedom', 'institutional_independence'][i % 5] as IntentStatement['policyArea'],
      })
    );
...
describe('analyzeRhetoricActionGap', () => {
  it('returns gaps only for areas with gap > 0.5', () => {
    const assessment = scoreStatements([
      ...
      makeStatement({ text: 'We support the rule of law and separation of powers and checks and balances', type: 'rhetoric', policyArea: 'rule_of_law' }),
      makeStatement({ text: 'I have absolute authority and total immunity and unlimited power', type: 'action', policyArea: 'rule_of_law' }),
    ]);
  ...
  it('returns empty array when no significant gaps', () => {
    const assessment = scoreStatements([
      makeStatement({ text: 'general policy discussion today', type: 'rhetoric', policyArea: 'rule_of_law' }),
      makeStatement({ text: 'general policy discussion today', type: 'action', policyArea: 'rule_of_law' }),
    ]);
    const gaps = analyzeRhetoricActionGap(assessment);
