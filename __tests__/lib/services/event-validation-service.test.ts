...
describe('negative control evaluations', () => {
  ...
  describe('NC-2: Biden 2022 P2 confirmation rate', () => {
    it('passes within 5–70% range', () => {
      expect(evaluateNc2BidenP2ConfirmRate(0.35).pass).toBe(true);
    });

    it('fails below 5%', () => {
      expect(evaluateNc2BidenP2ConfirmRate(0.04).pass).toBe(false);
    });
  ...
  describe('NC-3: Biden 2022 elevated weeks (tiered thresholds)', () => {
    ...
    it('fails at >14% for categories with ≥20 avg docs/week', () => {
      const data = [
        { category: 'civilService', elevatedCount: 8, totalWeeks: 52, avgDocsPerWeek: 30 },
      ];
