...
describe('OverviewStatusSummary', () => {
  it('renders stacked bar and legend for all statuses', () => {
    ...
    expect(screen.getByText('Consistent with norms: 500')).toBeDefined();
    expect(screen.getByText('Notable departure from norms: 65')).toBeDefined();
    expect(screen.getByText('Sustained departure from norms: 5')).toBeDefined();
  ...
  it('shows zero-count statuses in legend', () => {
    ...
    render(<OverviewStatusSummary statusCounts={statusCounts} />);
    expect(screen.getByText('Consistent with norms: 100')).toBeDefined();
    expect(screen.getByText('Notable departure from norms: 0')).toBeDefined();
    expect(screen.getByText('Sustained departure from norms: 0')).toBeDefined();
  });
