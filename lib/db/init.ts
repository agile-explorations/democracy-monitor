...
function restoreFromArchive(connectionString: string, force: boolean): void {
  ...
  execSync(
    `gunzip -c /tmp/${DOCS_CSV_FILENAME} | psql "${connectionString}" -c "\\copy documents(id, source_type, category, title, content, url, published_at, fetched_at, metadata, source_origin, case_id, speaker, content_type, embedded_at, retrieval_relevant, counting_scope) FROM STDIN WITH CSV HEADER"`,
    { stdio: 'inherit' },
  );
