...
export function draftRules(p2Count: number, totalDocs: number): string[] {
  const rules = [
    ...
    '2a. QUOTE DISCIPLINE: quote sparingly — quote only when the exact wording is',
    '    load-bearing, at most five quoted passages per answer; otherwise paraphrase',
    '    without quotation marks. Words in quotation marks must appear verbatim in',
    '    the document cited in the same sentence, and that sentence must name the',
    '    source document by title (or a clear short form of it) next to its [Doc N]',
    '    — naming the document forces you to check WHICH document the words came',
    '    from. Before finalizing, re-check each quoted phrase against the named',
    "    document's text; if you cannot confirm which document a phrase came from,",
    '    paraphrase it without quotation marks instead.',
