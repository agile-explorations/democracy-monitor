 ...
 */
const METADATA_ONLY_POPULATIONS: {
  population: string;
  column: 'source_type' | 'source_origin';
  value: string;
  mode: MetadataOnlyStats['mode'];
  hint: string;
}[] = [
  // Docket stubs retired to tracked_cases (#697): any court_opinion row in
  // documents is stub regrowth, not a marking backlog.
  {
...
export async function getMetadataOnlyClassification(): Promise<MetadataOnlyStats[]> {
  ...
  for (const p of METADATA_ONLY_POPULATIONS) {
    const filter =
    ...
    results.push({
      ...
      pass: p.mode === 'none-present' ? total === 0 : total === 0 || marked === total,
      hint: p.hint,
    });
  }
  return results;
...
/** CREC granules the composite fragment build (#704/#929) has not assessed:
 *  whole-day multi-topic records and granules where several members spoke
 *  (`speakerAmbiguous`, #927), without the `fragmentsAssessedV2` marker. Uses
 *  the build's own candidate predicate so the detector and the builder can
 *  never disagree; granule level (one row per category). `recentDays` limits
 *  the check to the weekly incremental window; omit for the standing check. */
export async function countUnfragmentedCrecGranules(recentDays?: number): Promise<number> {
  ...
  const recentFilter = recentDays
    ...
    : sql``;
  const rows = await db.execute(sql`
    SELECT count(DISTINCT metadata->>'granuleId') AS n
    FROM documents
    WHERE ${compositeCandidateSql()} ${recentFilter}`);
