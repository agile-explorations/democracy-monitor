...
_Originally assessed as "Partial Pass." Upgraded after API manual review assuming national account ($1K/year)._

| Dimension                     | Finding                                                                                                                                                                                                                                            |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **API access**                | National account: $1K/year, all 50 states + Congress. Bulk API eliminates per-bill query overhead.                                                                                                                                                 |
...
| **National search**           | `getSearchRaw(state=ALL)` — 2,000 results/page across entire national database. Boolean operators. Relevance scoring.                                                                                                                              |
...
| **Push API**                  | Real-time updates (15 min–4 hours). 25 change-detection flags per bill. Optional paid add-on.                                                                                                                                                      |
...
| Item                  | Cost                                   | Notes                                                             |
| --------------------- | -------------------------------------- | ----------------------------------------------------------------- |
| National API account  | $1,000/year                            | All 50 states + Congress                                          |
| Push API (optional)   | Additional subscription                | Real-time updates, 15 min–4 hours                                 |
| AI classification     | ~$2-5/session for batch classification | Title + description for ~100 election bills per high-volume state |
| Baseline construction | One-time bulk download                 | ~50 `getDataset` calls for 2017-2022 sessions across all states   |

### Revised verdict

**STRONG PASS.** With a national account, LegiScan provides:
...
- SAST cross-state tracking — a unique signal unavailable from any other source
- Bill text for Layer 2 AI classification and Layer 3 embedding analysis
- Bulk API makes baseline construction and ongoing monitoring operationally simple

The VRL validation step is deferred to a nice-to-have calibration exercise, not a launch blocker.
...
| ----- | -------------------------- | ------------------------------------ | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | LegiScan                   | elections                            | **Strong pass** (revised) | National account unlocks Bulk API + SAST cross-state tracking. Layer 1 uses metadata (no AI needed). ~20 election bills/week nationally. $1K/year. |
...
1. **Which new sources are viable for launch?**
   ...
   - **LegiScan** (Spike 1): Yes — national account ($1K/year) unlocks Bulk API + SAST. Anchor source for elections.
...
6. **Sprint R-S1 scope?**
   - P0: CourtListener integration (serves 3 categories), DOJ API, GovInfo/GAO API, LegiScan (elections anchor)
   - P1: FCC RSS, FEC API, IG RSS feeds
   - P2: oversight.gov scraping, VRL partnership (calibration dataset for LegiScan AI classification)
