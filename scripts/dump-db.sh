...
psql "${DB_URL}" -c "\copy (SELECT id, source_type, category, title, content, url, published_at, fetched_at, metadata, source_origin, case_id, speaker, content_type, embedded_at, retrieval_relevant, counting_scope FROM documents) TO STDOUT WITH CSV HEADER" | gzip > "${DOCS_CSV}"
