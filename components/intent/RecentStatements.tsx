    ...
    <div className="space-y-2">
      ...
      <ul className="space-y-1.5 text-xs">
        {statements.slice(0, 10).map((stmt, idx) => {
          const typeColor =
            stmt.type === 'action' ? 'text-blue-600 bg-blue-50' : 'text-purple-600 bg-purple-50';
          const scoreColor =
            stmt.score > 0.5
              ? 'text-red-600'
              : stmt.score < -0.5
          ...
          return (
            <li key={idx} className="flex items-start gap-2">
              <span
                className={`shrink-0 px-1.5 py-0.5 rounded text-[10px] font-medium ${typeColor}`}
              >
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-2 mt-0.5 text-slate-400">
                  <span>
                    {stmt.source} ({TIER_LABELS[stmt.sourceTier]})
                  </span>
                  <span>{stmt.date}</span>
