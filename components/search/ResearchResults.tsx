  ...
  return {
    ...
    a: ({ href, children }) => {
      ...
      if (citeMatch) {
        ...
        return (
          <span className="text-dm-accent text-xs font-medium" title={doc?.title}>
            [{idx}]
          </span>
        );
      }
...
function AnswerRenderer({ text, documents }: { text: string; documents: ResearchDocResult[] }) {
  ...
  return (
    <div className="text-sm leading-relaxed text-dm-text-secondary">
...
function ResearchDocCard({ doc }: { doc: ResearchDocResult }) {
  return (
    <div className="rounded border border-dm-border bg-dm-card p-3">
    ...
    <div className="space-y-6">
      {synthesizing && (
        <div className="rounded-lg border border-dm-accent/30 bg-dm-accent/5 p-4">
          <p className="text-sm text-dm-text-secondary">
            ...
            Generating answer from {docCount} document{docCount !== 1 ? 's' : ''}
            &hellip; Documents are shown below while you wait.
          </p>
        </div>
      )}
        ...
        <div className="rounded-lg border border-dm-border bg-dm-card p-4">
          <div className="mb-3">
            ...
            <p className="text-xs text-dm-muted mt-0.5">
              Based on {docCount} government document{docCount !== 1 ? 's' : ''}
              {result.dateRange.earliest !== 'unknown' &&
                ` \u00B7 ${result.dateRange.earliest} \u2013 ${result.dateRange.latest}`}
            </p>
          </div>
          <div className="prose prose-sm dark:prose-invert max-w-none text-dm-text-secondary">
      ...
      <div>
        <h3 className="text-xs font-semibold text-dm-muted uppercase tracking-wider mb-3">
          Government Record ({docCount} results, ranked by relevance)
        </h3>
