...
3. **Purge stubs**: `pnpm docs:purge-stubs` (dry run — pre-flight asserts every stub case exists in tracked_cases and no stub carries score/assessment rows), review the printed counts (~283k documents), then `--confirm`. No aggregate repair is needed; stubs were never scored. If snapshots ran between the local seed and the promote, the pre-flight will find late-arriving cases missing from tracked_cases — backfill them on prod with the seed's stub-fallback insert (universe aggregation from documents, no bulk join, `refreshed_at NULL` so the weekly refresh self-heals their dates), then re-run the dry run.

_Executed 2026-08-10 (v1.8.0)_: promote 202,664 + gap-fill 539 late cases → 203,203 tracked; purge deleted 283,331 stub rows; pre-flight caught the gap exactly as designed. Note: the promote's pre-backup `pg_dump` of prod exceeded its old 10-min timeout (DB >6GB) — timeout raised to 45 min; `--skip-backup` is acceptable when the weekly Monday dump is fresh. 4. **Drop local staging** tables when done (`search_docket`, `search_court`, `search_opinioncluster`). 5. The next weekly dump ships the new format; the format change is noted in the data dictionary (`table_tracked_cases`, `table_documents`).
