...
| Dimension                     | Metric                                                                                                         | Baseline Comparison                                       |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| **Filing Volume**             | Docket entries per category per week, by NOS code                                                              | z-score against baseline                                  |
...
These items emerged from the ChatGPT and Claude Code reviews. They are validated as good ideas but are not required for initial implementation. They are tracked in the separate `ARCHITECTURE_ROADMAP.md` document with full specifications and should be evaluated for inclusion as the system matures.

**Pass 1 pre-filtering with functional classifier**: Documents classified by Layer 1 as `financial_regulatory` or `cultural_ceremonial` are formulaic (SEC filings, cultural import determinations) and extremely unlikely to be relevant to erosion concerns. Skipping Pass 1 for these documents would reduce AI costs by ~15–20% with zero false-negative risk. Conceptually clean — Layer 1's structural classification informing Layer 2's scope. (Source: Claude Code review #4)

**Sprint 21 work: preservation vs. deprecation**: Sprint 21 added 56 operational keywords, the `admin-specific-keywords.ts` overlay system, and `getEffectiveKeywords()` pipeline integration. Under the new architecture, the overlay data remains valuable as annotation metadata (e.g., highlighting "DOGE" in documents), but the `getEffectiveKeywords()` pipeline integration — designed for scoring, not display — becomes unnecessary. The admin overlay's date-filtering complexity was built for scoring and can be simplified for annotation use. (Source: Claude Code review #7)

**Cross-category synchrony detection**: Small anomalies across many categories in the same week may indicate coordinated action. Already mentioned in Layer 1's "what it catches" list as "cross-category clustering." Future work could make this a first-class metric: count how many categories are simultaneously elevated, and treat multi-category elevation as its own signal above the sum of individual category signals. (Source: ChatGPT Red Team #4)

**Cross-source thematic convergence (Layer 3)**: Detect when multiple source types within a category drift in the same semantic direction simultaneously (e.g., court opinions, press releases, and FR documents all shifting toward similar themes). Deferred from Sprint R-S1 because court opinions, press releases, and legislative text occupy fundamentally different regions of embedding space — "same direction" between different linguistic registers may not carry interpretable signal. Requires empirical investigation: compute between-source-type cosine distances during baseline periods and determine whether meaningful cross-source thematic signals exist. If validated, this becomes the Layer 3 analog of Layer 1's cross-source structural convergence. (Source: Claude Code source expansion review)

**LegiScan per-state session-aware baselines**: The R-S1 implementation uses national aggregate baselines (sum across 50 states), which naturally smooths per-state session seasonality but may be dominated by high-volume states (CA, TX, NY, FL). Per-state baselines with session-relative time normalization would account for different session calendars (year-round, biennial, special sessions, carryover rules) and detect state-specific anomalies that wash out in national aggregation. Significant calibration effort — 50 state-specific baseline models. Monitor national aggregate for concentration effects first. (Source: ChatGPT/Claude Code source expansion review)

**Coverage health monitoring** _(elevated to Sprint R-S1 requirement)_: With 7+ source types, distinguishing "no data because nothing happened" from "no data because the pipeline is broken" from "no data because the source changed" becomes critical — a broken pipeline could be misinterpreted as "institutional silence," which is the worst possible confusion for this project. Track per-source ingestion success, latency, schema changes, and missingness. Minimum viable implementation for R-S1: per-source-type document count per day, with alerting when a source goes silent for >2× its expected publication cadence. Track DOJ/FBI/ICE topic taxonomy changes separately from content shifts. The "Other/unclassified" functional bucket ratio should also be monitored — a spike in unclassified documents may indicate the government is changing metadata conventions. Full operational dashboard deferred to R-F but basic monitoring ships with source expansion. (Source: ChatGPT Red Team #5, elevated per ChatGPT/Claude Code source expansion review)
