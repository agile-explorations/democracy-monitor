 ...
 * litigation for a category's Litigation panel (#696). Serves tracked_cases
 * only (zero CourtListener calls); Redis-cached 24h per page.
 */
...
async function queryCases(key: string, status: string, page: number): Promise<CasesPayload> {
  ...
  const rows = (
    await db.execute(sql`
      SELECT case_id, case_name, court_name, docket_number, nature_of_suit,
        date_filed::text, date_terminated::text, date_last_filing::text, status, posture
      FROM tracked_cases
      WHERE categories @> ${JSON.stringify([key])}::jsonb ${statusCond}
  ...
  const counts = (
    await db.execute(sql`
      ...
      FROM tracked_cases WHERE categories @> ${JSON.stringify([key])}::jsonb`)
