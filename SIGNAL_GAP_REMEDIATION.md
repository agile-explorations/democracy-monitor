...
**3-month rolling window constraint**: GDELT DOC 2.0 only makes the last 3 months searchable. The 231K rhetoric documents already fetched from the Trump 2025 backfill are the corpus — they cannot be re-fetched with `sourcecountry:US` applied for anything older than 3 months ago. Two mitigation options:
...
33. **GDELT's 3-month rolling window means historical rhetoric data cannot be re-fetched.** The 231K rhetoric documents from the Trump 2025 backfill are the corpus as-is. Improvements to GDELT query parameters (like `sourcecountry:US`) only apply to future fetches. For historical data, international noise must be filtered at the scoring/classification stage rather than the fetch stage. This also means GDELT-sourced baselines for Biden 2021/2022 and Trump 2017/2018 could not be re-fetched today — those historical rhetoric documents are permanently archived as fetched.
