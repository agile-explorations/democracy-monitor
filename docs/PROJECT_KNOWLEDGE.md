...
- R-CREDIBILITY (#667 #700 #711 #714 #744 #741 #772 + #796, milestone 126, v1.17.0–v1.17.3, 2026-08-28, last outreach gate): snapshot self-heals unscored docs before G1a (real cause of the digest holds: late-published docs outside the sweep); deterministic number check + digest hold + `narratives:verify` (two stored miscounts regenerated); empty-synthesis redraw; passages skip every masthead (+ `verify:enrichment-sql`, which caught a Postgres regex bound); SCOTUS emergency docket + `cl_cluster_ledger` + trailing pass + revision dedup with cascade (592 rows, repair run); 400-row annotation audit (33 corrections); era-sliced verdict rates on the methodology page + `audit:symmetry` swap audit. Retro in DECISIONS.md.
