...
- **AI narratives** — 3-pass multi-model pipeline (Opus draft → GPT-4o feedback → Opus revision), weekly summaries, term summaries, editorial transparency. Spec-compliant prompts with safety-net criteria for "why this might matter," small-sample caveats, counter-argument limits. validate:narratives QA script. Pre-computed trajectory summaries, P2 reasoning prominence, document markdown links, 4000-char content excerpts. (Sprints R4a, R-NAR1, R-NAR3, R-NAR)
...
- **Detection architecture transition** — L2-only convergence (L1/L1v2/L3 demoted to descriptive context), absolute P2 thresholds (no baseline z-score comparison), Divergent status retired, P2 reasoning enhancements (contextual connections, mechanism identification, CREC response). Threshold tuning pending (#419). (Sprint R1-DET, Issues #410–#418).
- **Terminology cleanup** — Renamed "convergence" → "concern", "layer2" → "document-review" across services/types/components/tests/docs. Removed Divergent from charts (Y-axis 0-3→0-2), deleted ConvergenceIndicator, simplified ConcernHeader. (Sprint R1-CLN, Issues #420–#431).
- **Researcher data access** — /data page with CSV download cards, full database download, API docs, CSV column reference. CSV exports flatten jsonb columns into individual fields. Descriptive metrics chart (structural/AI/thematic) with toggle on category detail. (Sprint R-DATA1, Issues #434–#439).
