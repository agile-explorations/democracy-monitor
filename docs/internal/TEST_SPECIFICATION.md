...
- **Content completeness by source type**
  - Assert `validate:ingest` reports null-content counts per `source_type`, broken down by fixable vs non-fixable.
...
- **Post-backfill embedding reset**
  ...
  - After `pnpm embeddings:backfill`, assert those 5 documents have `embedded_at IS NOT NULL`.
