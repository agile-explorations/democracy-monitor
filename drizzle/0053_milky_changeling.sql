ALTER TABLE "documents" ADD COLUMN IF NOT EXISTS "search_rank_vector" "tsvector";--> statement-breakpoint
-- Trigger keeps search_rank_vector current on every write path (app upserts,
-- backfills, promotes). Matching still uses the full generated search_vector;
-- this compact vector exists only for ts_rank ordering (#702/#703).
...
DROP TRIGGER IF EXISTS trg_documents_search_rank_vector ON "documents";--> statement-breakpoint
