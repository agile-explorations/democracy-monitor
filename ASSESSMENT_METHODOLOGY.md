...
- **Category relevance filtering (Press Freedom, Government Information Availability)** — Two categories whose retrieval terms match large volumes of unrelated Federal Register text carry a validated relevance filter: documents whose subject is not the category's are excluded from its counts and assessment (July 2026 for Press Freedom; September 2026 for Government Information Availability, where a measurement audit had found a random sample on-topic zero times in one hundred). Every filter is derived against an owner-adjudicated labeled sample, gated on a fresh holdout with zero false drops, enforced by standing regression tests, and audited weekly by an LLM pass over the drop ledger. The September 2026 correction changed twenty weekly statuses, all downward; the change is recorded on the public reversals ledger with the evidence trail. Misrouted documents found to carry real signal were re-assessed under their correct categories rather than discarded.
