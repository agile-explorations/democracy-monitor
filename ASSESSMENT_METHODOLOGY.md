...
**Three document populations.** The stored record is one corpus; the departure analysis and search are two independent uses of it. _Searchable_ — every document with a body that is not a superseded revision; this is what the Search page, research answers, and the embedder operate on, and topic relevance is not a search criterion. _Counting_ — the searchable documents that are relevant to their category and inside the documented counting scope; this is what document counts, structural statistics, silence, drift, and baselines describe. _Analysis evidence_ — the documents that are detection evidence for a category; this is what scoring, AI document review, narratives, and validation read. A document a relevance filter annotates off-topic, or that no router places, leaves the counting and evidence populations but stays searchable, labelled "Not routed to a category"; nothing is deleted, and every exclusion is recorded in the public drop ledger.
